      ...
      <div class="lg:col-span-1">
        <div class="sticky top-6">
          ...
          >
            {{
              hasSignificantScore
                ? "Toon gedetailleerde match!"
                : "Beantwoord meer vragen"
            }}
          </button>
...
const currentScores = computed(() => {
  ...
  const percentageScores = {
    [TripName.Cervinia]: Math.max(
      0,
      Math.min(
        100,
        maxScores[TripName.Cervinia] > 0
          ? (scores[TripName.Cervinia] / maxScores[TripName.Cervinia]) * 100
          : 0
      )
    ),
    [TripName.Nassfeld]: Math.max(
      0,
      Math.min(
        100,
        maxScores[TripName.Nassfeld] > 0
          ? (scores[TripName.Nassfeld] / maxScores[TripName.Nassfeld]) * 100
          : 0
      )
    ),
    [TripName.LittleKidsWeek]: Math.max(
      0,
      Math.min(
        100,
        maxScores[TripName.LittleKidsWeek] > 0
          ? (scores[TripName.LittleKidsWeek] /
              maxScores[TripName.LittleKidsWeek]) *
              100
          : 0
      )
    ),
